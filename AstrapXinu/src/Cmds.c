...
void CMDS_Process(void)
{
    ...
    case CMDSM_WAITFORLINE:
        ...
        if( cmds_InpPtr[0] == 'a' )
            ...
            signal_done = cmds_R();
        else if( cmds_InpPtr[0] == 's' && cmds_InpPtr[1] == 'm')
        	signal_done = cmds_SM();
...
static bool cmds_SM( void )
{
	...
	U2_Print32("sccmram:    ", (uint32_t)&_sccmram);
	U2_Print32("eccmram:    ", (uint32_t)&_eccmram);
	U2_Print32("estack:     ", (uint32_t)&_estack);
	U2_Print32("bss_start:  ", (uint32_t)&__bss_start__);
	U2_Print32("bss_end:    ", (uint32_t)&__bss_end__);
	U2_Print32("&this_var:  ", (uint32_t)&this_var);
