...
void U2_Process( void )
{
    ...
    case SERO_STATE_GETJOB:                                                              // Looking for items on the Queue
        ...
        switch( serd_active_Qitem->sr_otype )                                            // sr_otype dictates the action
        {
          ...
          case SERO_TYPE_32N:     ItoH(serd_active_Qitem->sr_dval, serd_databuf);        // 32-bit to Hex Data Conversion.  Place in databuf
                                  break;
          ...
          case SERO_TYPE_16:      ItoH16(serd_active_Qitem->sr_dval, serd_databuf);      // 16-bit to Hex Data Conversion.  Place in databuf
          ...
          case SERO_TYPE_16N:     ItoH16(serd_active_Qitem->sr_dval, serd_databuf);      // 16-bit to Hex Data Conversion.  Place in databuf
                                  break;
        }
...
void U2_Print16( const char *pstr, u16 val )
{
    U2_Send( SERO_TYPE_16, (char *)pstr, 0, val );
}

void U2_Print16N( const char *pstr, u16 val )
{
    U2_Send( SERO_TYPE_16N, (char *)pstr, 0, val );
}
